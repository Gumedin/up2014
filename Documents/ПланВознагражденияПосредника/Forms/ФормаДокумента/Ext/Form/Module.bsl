...
Процедура ЗаполнитьИзКалькулятораНаСервере( Калькулятор )
	...
	Док.Начисления.Загрузить( Калькулятор.Начисления.Выгрузить());
...
Процедура ПодтвердитьЗаполнение(Калькулятор, ДопПараметры ) Экспорт
	...
	ПоказатьВопрос(ОпОповещения, "Заполнить текущий план вознаграждения из выбранного калькулятора?",
...
Процедура ЗаполнитьИзКалькулятора(Команда)
	...
	Если Объект.Проведен Тогда
		ПоказатьПредупреждение(,"Заполнить из калькулятора можно только непроведенный план вознаграждения!");
		Возврат;
	КонецЕсли;
	
	ОпОповещения = Новый ОписаниеОповещения("ПодтвердитьЗаполнение", ЭтаФорма );
	ПоказатьВводЗначения( ОпОповещения, Калькулятор,, Тип("СправочникСсылка.КалькуляторПланаВознаграждения"));
КонецПроцедуры
