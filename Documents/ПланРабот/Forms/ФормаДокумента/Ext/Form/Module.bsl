...
//	СуммаПоСмете = СуммаПоСмете( Объект.Ссылка );
	СуммаПоСмете = УП_СметаПроекта.ПолучитьОборотБюджетаПоСтатье(
		Справочники.СтатьиСметы.ФОТ_ПП,
		Объект.ЗадачаПроекта.Владелец,
		Объект.ЗадачаПроекта,
		Перечисления.ТипСуммыБюджета.План);
	// 
	ЗаполнитьДеревоФР();
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.БезКоэффициентовФОТ.Доступность = СуммаПоСмете <> 0;
		//НЕ СметаЗадачиПроведена( Объект.Ссылка  );
...
&НаСервере
Процедура ЗаполнитьПоЗадачеСервер() Экспорт
	Документ = РеквизитФормыВЗначение("Объект");
	
	ПериодЗадачи = Новый Структура();
	ПериодЗадачи.Вставить("ДатаНачала", Объект.ЗадачаПроекта.НачалоРабот);
	ПериодЗадачи.Вставить("ДатаОкончания", Объект.ЗадачаПроекта.ОкончаниеРабот);
	
	сКалендарь = УП_ПланыРаботПоПроектам.РабочиеДниПоМесяцам(ПериодЗадачи.ДатаНачала, ПериодЗадачи.ДатаОкончания);
	
	тзИсполнители = Объект.ЗадачаПроекта.Исполнители.Выгрузить();
	тзИсполнители.Свернуть("Должность", "Загрузка");
	Для Каждого исполнитель из тзИсполнители Цикл 
		
		УП_ПланыРаботПоПроектам.ОбработатьИзмененияФронтаРабот( 
			Документ, ПериодЗадачи, 
			исполнитель.Загрузка / 100, исполнитель.Должность, 
			ИсполненВключая, "Установить", сКалендарь, "РЧасов");
	КонецЦикла;
...
&НаКлиенте
Процедура ЗаполнитьПоЗадачеПодтверждение(Результат, Параметры) Экспорт
	Если Результат <> КодВозвратаДиалога.Отмена Тогда
		Если Результат = КодВозвратаДиалога.Да Тогда
			Объект.ФронтРабот.Очистить();
		КонецЕсли; 
		ЗаполнитьПоЗадачеСервер();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоЗадаче(Команда)
	ОпОповещения = Новый ОписаниеОповещения("ЗаполнитьПоЗадачеПодтверждение", ЭтаФорма);
	ПоказатьВопрос(ОпОповещения, "Очистить текущий план перед расчетом по исполнителям?",
