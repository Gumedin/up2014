...
Функция СписокПериодовРаспределения_ПоЗадачам(Вариант, ЗадачаПроекта)
	...
	Если Объект.Фиктивный Тогда
		...
		Если ДатаНачалаРабот<ЗадачаПроекта.ОкончаниеРабот Тогда
			Сообщить("Дата начала работ (текущая дата) меньше даты окончания работ по задаче ", 10);
		КонецЕсли;		
	Иначе	
...
Процедура ЗаполнитьЗадачами_НаСервере()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(Контрагент) ИЛИ Контрагент = Объект.Контрагент Тогда
...
Процедура ОтгрузАванс30Проц(Команда)
	...
	Для Каждого ЗадачаПроекта из Объект.ЗадачиПроекта Цикл 
		...
		ПараметрыОтбора = Новый Структура;
		ПараметрыОтбора.Вставить("ЗадачаПроекта", ЗадачаПроекта.ЗадачаПроекта); 
		ЭтапГрафикаОтгрузки = Объект.ЭтапыГрафикаОтгрузки.НайтиСтроки(ПараметрыОтбора)[0];	
		ЭтапГрафикаОтгрузки.ПроцентОтгрузки = 30;
...
Процедура РапределитьОтгрузку(Вариант)
	...
	Объект.ЭтапыГрафикаОтгрузки.Очистить();
	Для Каждого ЗадачаПроекта из Объект.ЗадачиПроекта Цикл  
		
		СписокПериодовПоЗадачам = СписокПериодовРаспределения_ПоЗадачам(Вариант, ЗадачаПроекта.ЗадачаПроекта);
		
		РаспределитьПоВариантуОтгр( СписокПериодовПоЗадачам , ЗадачаПроекта.ЗадачаПроекта);
		//Формирование сумм и процентов по задачам проекта 
		БалансПроцентовИСуммОтгр_ПоЗадачам(1, ЗадачаПроекта.Сумма, ЗадачаПроекта.ЗадачаПроекта);
	КонецЦикла;
	///////////////////////////////////////////////////////
...
//Изменено 14.05.2015 Гумединым А.Г. по задаче #129069
&НаСервере
Процедура РаспределитьПоВариантуОтгр( СпПериодов , ЗадачаПроекта)
	СпрОб = РеквизитФормыВЗначение("Объект");
...
Процедура БалансПроцентовИСуммОтгр_ПоЗадачам(Вариант = 1, Сумма, ЗадачаПроекта)
	...
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("ЗадачаПроекта", ЗадачаПроекта); 
	ГрафикиОтгрузки = Объект.ЭтапыГрафикаОтгрузки.НайтиСтроки(ПараметрыОтбора);
	
	ИтогоПроцент = 0;
	Для Каждого ГрафикОтгрузки ИЗ ГрафикиОтгрузки Цикл
		ИтогоПроцент = ИтогоПроцент + ГрафикОтгрузки.ПроцентОтгрузки;		
	КонецЦикла;
	
	Если ГрафикиОтгрузки.Количество() <> 0 Тогда
		//
		Стр = ГрафикиОтгрузки[ГрафикиОтгрузки.Количество()-1];
		Стр.ПроцентОТгрузки = Стр.ПроцентОтгрузки + (100-ИтогоПроцент);
		// чтобы округлить
...
Процедура ПересчетСуммГрафикаПлатежейОтгр_ПоЗадачам(Вариант = 1, Сумма, ЗадачаПроекта)
	...
	Если Объект.ЭтапыГрафикаОтгрузки.Количество() <> 0 Тогда
		...
		// последняя строка
		Стр = ГрафикиОтгрузки[ГрафикиОтгрузки.Количество()-1];
		Стр.СуммаОтгрузки = Сумма;
		
		// вариант 1, сумма от процентов
		Для Н = 0 ПО (ГрафикиОтгрузки.Количество() - 2) Цикл
			СтрН = ГрафикиОтгрузки[Н];
			Если Вариант = 1 Тогда
				СтрН.СуммаОтгрузки	= Сумма * СтрН.ПроцентОтгрузки /100;
			Иначе
				СтрН.ПроцентОтгрузки	= СтрН.СуммаОтгрузки/Сумма * 100;
			КонецЕсли;
...
Процедура ЭтапыГрафикаОтгрузкиСуммаОтгрузкиПриИзменении(Элемент)
	...
	Стр = Элементы.ЭтапыГрафикаОтгрузки.ТекущиеДанные;
	//
	Стр.ПроцентОтгрузки = Стр.СуммаОтгрузки / ЗадачаПроекта.Сумма * 100;
...
&НаКлиенте
Процедура ПоГрафикуОтгрузки(Команда)
	// Вставить содержимое обработчика.
	Если Объект.ЭтапыГрафикаОплаты.Количество() <> 0 Тогда
		Если Вопрос("Текущее распределение будет очищено, продолжить?", РежимДиалогаВопрос.ДаНет)=КодВозвратаДиалога.Да	Тогда
		Иначе
	...
	Объект.ЭтапыГрафикаОплаты.Очистить();
	Для Каждого ЭтапОтгрузки ИЗ Объект.ЭтапыГрафикаОтгрузки	Цикл
		СтрокаСостава = Объект.ЭтапыГрафикаОплаты.Добавить();
		СтрокаСостава.ЗадачаПроекта = ЭтапОтгрузки.ЗадачаПроекта;
		СтрокаСостава.ДатаПлатежа =  ДобавитьМесяц(ЭтапОтгрузки.ДатаОтгрузки, 1);
		СтрокаСостава.СуммаПлатежа = ЭтапОтгрузки.СуммаОтгрузки;
		СтрокаСостава.ПроцентПлатежа = ЭтапОтгрузки.ПроцентОтгрузки;
		СтрокаСостава.Комментарий = ЭтапОтгрузки.Комментарий;
	КонецЦикла; 	
КонецПроцедуры
...
Процедура ЗадачиПроектаПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные = Неопределено ИЛИ ПоказатьВсе Тогда
		...
		Элементы.ЭтапыГрафикаОтгрузки.ОтборСтрок = Неопределено;
    Иначе
		...
		Элементы.ЭтапыГрафикаОтгрузки.ОтборСтрок = Новый ФиксированнаяСтруктура("ЗадачаПроекта", ЗадачаПроекта);
	КонецЕсли;	
КонецПроцедуры
