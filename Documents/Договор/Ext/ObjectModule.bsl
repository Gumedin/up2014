...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда  
		...
		//Изменено по задаче #133737 от 25.10.2018 ГумединАГ 
		Для Каждого Реализация Из ЭтотОбъект.ЭтапыГрафикаРеализации Цикл 
				Если Реализация.ДатаРеализации < Реализация.ЗадачаПроекта.НачалоРабот
					ИЛИ Реализация.ДатаРеализации > Реализация.ЗадачаПроекта.ОкончаниеРабот Тогда 
					Сообщить("В графике отгрузки задачи " + Реализация.ЗадачаПроекта + " имеются даты вне периода действия задачи.");
					Отказ = Истина;
					Возврат;
				КонецЕсли;
		КонецЦикла;
