...
Процедура НачислитьНаСервере()
	...
	Запрос.Текст = 
		...
		|	ФондПодразделений.Сотрудник КАК Сотрудник
		...
		|	ВТ_БазаПоЗадачам.ЗадачаПроекта КАК ЗадачаПроекта,
		...
		|	ВТ_БазаПоЗадачам.Сотрудник КАК Сотрудник
		|ИЗ
	...
	РасчетИтоговПоПроектам();
КонецПроцедуры

&НаСервере
Процедура РасчетИтоговПоПроектам()
	тзИтоги = Объект.Начисления.Выгрузить();
	тзИтоги.Свернуть("ЗадачаПроекта, Процент", "БазаРаспределения, Сумма");
	ИтогиПоПроектам.Загрузить(тзИтоги);
КонецПроцедуры	
