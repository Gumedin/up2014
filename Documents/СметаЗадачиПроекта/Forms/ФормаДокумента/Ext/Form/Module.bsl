...
&НаСервереБезКонтекста
Процедура ПересчетСметыЗадачи_НаСервере( ЗадачаПроекта, П, С, ИзмД, сРезультат )
...
Процедура ПересчетСметыЗадачи()
	...
	ПересчетСметыЗадачи_НаСервере( Объект.ЗадачаПроекта, сПар, сСуммы, сИзмД, сРезультат );
	
	// 
	Для Каждого Стр ИЗ Объект.Расчет Цикл
		ИмяСтатьи = ИмяСтатьи( Стр.Статья); 
		//Стр.Сумма = сРезультат.Получить( Стр.ИмяСтатьи );
		Стр.Сумма = сРезультат.Получить( ИмяСтатьи );
	КонецЦикла;
КонецПроцедуры
