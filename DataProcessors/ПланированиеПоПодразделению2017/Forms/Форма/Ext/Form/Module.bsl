...
Процедура ЗаполнитьПосещения_ПоПланамРабот(ПериодПланирования )
	...
	СотрудникиПодразделения		= УП_КадрыСервер.СотрудникиПодразделенияЗаПериод( Подразделение, ПериодПланирования, Ложь,, ВключатьНеДоступныхСотрудников );
	
	Для Каждого Посещение ИЗ РезультатЗапроса Цикл
		// если в посещении не указана тарифная ставка, заполняем её по условиям		
		ТарифнаяСтавка 				= ТарифнаяСтавкаПосещения( Посещение, СотрудникиПодразделения, НачалоПериода );
		Посещение.ТарифнаяСтавка 	= ТарифнаяСтавка;
		
		Отбор = Новый Структура( "ФизическоеЛицо,ТарифнаяСтавка", Посещение.ФизическоеЛицо, ТарифнаяСтавка);
		...
		Если Стр.Количество() = 0 Тогда 
			...
			Исполнитель.СтавкаФОТ 		= СтавкаФОТ( Подразделение, ТарифнаяСтавка, НачалоПериода);
			Исполнитель.КодТС 			= КодТарифнойСтавки( ТарифнаяСтавка );
	...
	// может быть несколько должностей с одинакоковой тарифной ставкой
