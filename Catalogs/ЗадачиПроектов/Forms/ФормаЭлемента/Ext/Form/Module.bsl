...
//Добавлено 18.05.2018 Гумединым А.Г. по задаче #129069
&НаСервере
Процедура ЗаполнитьГрафикиОтгрузкиПоДоговорамНаСервере()	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДоговорЭтапыГрафикаОтгрузки.ДатаОтгрузки КАК ДатаОтгрузки,
		|	ДоговорЭтапыГрафикаОтгрузки.СуммаОтгрузки КАК СуммаОтгрузки,
		|	ДоговорЭтапыГрафикаОтгрузки.ПроцентОтгрузки КАК ПроцентОтгрузки,
		|	ДоговорЭтапыГрафикаОтгрузки.Комментарий КАК Комментарий
		|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Строка = Объект.ЭтапыГрафикаОтгрузкиПоДоговорам.Добавить();
		Строка.ДатаОтгрузки = ВыборкаДетальныеЗаписи.ДатаОтгрузки;
		Строка.СуммаОтгрузки = ВыборкаДетальныеЗаписи.СуммаОтгрузки;
		Строка.ПроцентОтгрузки = ВыборкаДетальныеЗаписи.ПроцентОтгрузки;
...
//Добавлено 18.05.2018 Гумединым А.Г. по задаче #129069
&НаКлиенте
Процедура ЗаполнитьГрафикиОтгрузкиПоДоговорам(Команда)
	ЗаполнитьГрафикиОтгрузкиПоДоговорамНаСервере();
КонецПроцедуры
