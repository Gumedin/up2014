...
&НаСервере
Функция СформироватьЗапросПоРегистру_Уст()
	...
	Текст = 
		...
		|	БюджетПоМесяцамОбороты.ЗадачаПроекта.Владелец.Куратор КАК Куратор,
		...
		|	ВЫБОР
		|	КОГДА БюджетПоМесяцамОбороты.ЗадачаПроекта.НачалоРабот = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	...
	Текст = Текст + "
		...
		|				И Месяц >= &НачалоПериода
		|				И Месяц <= &КонецПериода
		|				И (ВЫБОР
		...
		|	БюджетПоМесяцамОбороты.ЗадачаПроекта.Владелец.Куратор,
	...
	Запрос.УстановитьПараметр("ГодЗадачи", 			ГодЗадачи);
	Запрос.УстановитьПараметр("ВсеГодыЗадач", 		ГодЗадачи = 0);
	...
	// 0
	Запрос.УстановитьПараметр("ВсеКураторы", 		НЕ ЗначениеЗаполнено( Кураторы ));
	Запрос.УстановитьПараметр("СпКураторов", 		Кураторы);
	// 0
...
Функция СформироватьЗапросПоРегистру()
	...
	Текст = 
		...
		|	БюджетПоМесяцамОбороты.ЗадачаПроекта.ТиповаяЗадача КАК ТиповаяЗадача,
		|	БюджетПоМесяцамОбороты.ЗадачаПроекта.ТиповаяЗадача.ВидТиповойЗадачи КАК ВидЗадачи,
		|	БюджетПоМесяцамОбороты.ЗадачаПроекта.ГодЗадачи КАК ГодЗадачи,
		|	БюджетПоМесяцамОбороты.ЗадачаПроекта.ТиповаяЗадача.ВидНоменклатуры 	КАК ВидНоменклатуры,";
	...
	Текст = Текст + "
		...
		|				И Месяц >= &НачалоПериода
		|				И Месяц <= &КонецПериода
		|				И (ЗадачаПроекта.ГодЗадачи = &ГодЗадачи ИЛИ &ВсеГодыЗадач )
		|				И ЗадачаПроекта.Владелец.Статус В (&СтатусыПроектов)
		|				И (&ВсеМенеджеры
		|					ИЛИ ЗадачаПроекта.Владелец.МенеджерПроекта В (&СпМенеджеров))
		|				И (&ВсеКураторы
...
Процедура ВывестиСекциюОтчета(ТабДок, Макет, Уровень = 0, ГоризСтр, СписокСтатей, сПараметры )
	...
	// кураторы
	Если ДетКуратор Тогда
		ОбластьГД	= Макет.ПолучитьОбласть(ГоризСтр + "|Куратор");
...
Процедура ПечатьОтчета( ТабДок )                  
	...
	// вывод 
	//Макет		= Отчеты.ПланФакт.ПолучитьМакет( "ПланФакт" );
	Макет		= РеквизитФормыВЗначение("Отчет").ПолучитьМакет( "ПланФакт" );
	сПараметры = Новый Структура;
	сПараметры.Вставить("ГодПроекта", 		ГодПроекта);
	Если ЗначениеЗаполнено( ГодЗадачи ) Тогда
		сПараметры.Вставить("ГодЗадачи", 		", задачи " + ГодЗадачи + "г.");
	КонецЕсли;
	сПараметры.Вставить("ПечПериод", "За период: " +		ПредставлениеПериода( Период.ДатаНачала, Период.ДатаОкончания, "ФП=Истина"));
	...
	тзПроекты.Свернуть( "Проект,Куратор,ДиректорПроекта,КлиентМенеджер", СтрокаСвертки );
	...
	Для Каждого СтрПр ИЗ тзПроекты Цикл
		...
		сПараметры.Вставить("Куратор", 			СтрПр.Куратор );
