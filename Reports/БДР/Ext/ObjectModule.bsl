...
//	КонецЕсли;;
	...
	Для Каждого итераторГод Из тзПоГодам Цикл
		...
		Для Каждого итераторПериод Из тзПоПериодам Цикл
			...
			тзРезультатЗаПериод = тзРезультат.Скопировать(Отбор);
			
			Если итераторПериод.Месяц <> Null Тогда
				Для Каждого СтрСметы ИЗ сСметы Цикл
					ОтборСтатья = Новый Структура();
					ОтборСтатья.Вставить("Статья", СтрСметы.Значение.Статья);
					
					рез = тзРезультатЗаПериод.НайтиСтроки(ОтборСтатья);
