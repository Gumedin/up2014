...
// структура (иерархия) итоговых статей сметы - старая версия
&НаСервере
...
// структура (иерархия) итоговых статей сметы - новая версия
&НаСервере
Процедура ПолучитьСтруктуруСметыЗадачиА(СтруктураСметыЗадачи, Год = Неопределено, СтатьяРодитель = Неопределено ) Экспорт
	ГодСметы 	= ?( Год = Неопределено, Год( ТекущаяДата()), Год );
	СтрСметы	= РегистрыСведений.СтруктураСметыПоГодам.ПолучитьПоследнее( Дата(Год,1,1)).СтруктураСметы;
	Если СтрСметы.Пустая() Тогда 
		Возврат;
...
Функция ПечатьСметы(МассивОбъектов, ОбъектыПечати, ИмяМакета = "", ТипИсточника) Экспорт
	...
	//
	Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщиеМакеты.ПФ_MXL_Смета");
	// шапка
...
Функция ПолучитьОборотБюджетаПоСтатье( СтатьяСметы, 
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.БюджетПоМесяцам
		|ГДЕ
		|	ЗадачаПроекта.Владелец = &Проект
		|	И ТипСуммы = &ТипСуммы
		|	И СтатьяСметы = &СтатьяСметы";
	
	Запрос.УстановитьПараметр("Проект", 			Проект);
	Запрос.УстановитьПараметр("СтатьяСметы", 		СтатьяСметы);
	Запрос.УстановитьПараметр("ТипСуммы", 			ТипСуммы);
	
	Если НЕ ЗадачаПроекта = Неопределено Тогда
		Запрос.Текст = Запрос.Текст + "	И ЗадачаПроекта = &ЗадачаПроекта";   
		Запрос.УстановитьПараметр("ЗадачаПроекта", ЗадачаПроекта);
	КонецЕсли;
	
	Если НЕ ВидПлана = Неопределено Тогда
		Запрос.Текст = Запрос.Текст + "	И ВидПлана = &ВидПлана"; 
...
&НаСервере
//Подбор сметы для Задачи
Функция ПодборСметыЗадачи(ЗадачаПроекта) Экспорт
	СметаЗадачиПроекта = Неопределено;
	...
	Если ТаблицаСмет.Количество() > 0 Тогда
		//Берем первую проведенную смету
		СметаЗадачиПроекта = ТаблицаСмет[0].Ссылка; 	
	КонецЕсли;
	
	Возврат СметаЗадачиПроекта;
КонецФункции	
//Скопировано из СметыЗадачиПроекта
&НаСервере
Процедура ПересчетСметыЗадачи_НаСервере( ЗадачаПроекта, сРасчета, П, С, ИзмД, сРезультат ) Экспорт 
...
&НаСервере
Процедура ПодготовкаФактБюджета(сСметы, сСтатей, пСтатей, oСтатей, ЗадачаПроекта) Экспорт
	СметаЗадачиПроекта = ПодборСметыЗадачи(ЗадачаПроекта);
	Если СметаЗадачиПроекта = Неопределено Тогда 
		Сообщить("Не найдена проведенная смета для задачи " + ЗадачаПроекта);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	//сСметы = новый Структура;
	ПолучитьСтруктуруСметыЗадачиА(сСметы, ЗадачаПроекта.ГодЗадачи );
...
&НаСервере
Процедура ПроведениеФактБюджета(сСметы, сСтатей, oСтатей, Движения, ЗадачаПроекта, Месяц, Дата) Экспорт
	//Пересчет
	ПересчетСметыЗадачи( сСметы, сСтатей );
	
	//Бюджет факт
	Для Каждого статья Из сСтатей Цикл
		Если сСтатей[статья.Ключ] <> 0 Тогда 
			Движение = Движения.БюджетПоМесяцам.Добавить(); 
			Движение.Период 		= Дата;
			Движение.ЗадачаПроекта 	= ЗадачаПроекта;
			Движение.СтатьяСметы 	= oСтатей[статья.Ключ];
			Движение.ТипСуммы 		= Перечисления.ТипСуммыБюджета.Факт;  // факт
			Движение.Месяц 			= НачалоМесяца(Месяц);
			Движение.Сумма          = сСтатей[статья.Ключ];	
			Движение.РКО			= Истина;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
