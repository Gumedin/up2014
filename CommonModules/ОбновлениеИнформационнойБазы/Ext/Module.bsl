...
Функция ВыбратьСсылкиДляОбработки(Очередь, ПолноеИмяОбъекта, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если МетаданныеОбъекта = Метаданные.Справочники.ЗадачиПроектовПрисоединенныеФайлы Тогда
		Табл = Новый ТаблицаЗначений;	
		Табл.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("СправочникСсылка.ЗадачиПроектовПрисоединенныеФайлы"));
		Возврат Табл;
	ИначеЕсли МетаданныеОбъекта = Метаданные.Справочники.ИнцидентыПрисоединенныеФайлы Тогда	
		Табл = Новый ТаблицаЗначений;	
		Табл.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("СправочникСсылка.ИнцидентыПрисоединенныеФайлы"));
		Возврат Табл;
	ИначеЕсли МетаданныеОбъекта = Метаданные.Справочники.ПроектыПрисоединенныеФайлы Тогда
		Табл = Новый ТаблицаЗначений;	
		Табл.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("СправочникСсылка.ПроектыПрисоединенныеФайлы"));
		Возврат Табл;	
	ИначеЕсли МетаданныеОбъекта = Метаданные.Справочники.ПосещенияКонтрагентовПрисоединенныеФайлы Тогда
		Табл = Новый ТаблицаЗначений;	
		Табл.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("СправочникСсылка.ПроектыПрисоединенныеФайлы"));
		Возврат Табл;	
	КонецЕсли;
...
Функция ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Для каждого ОбрабатываемыйТип Из ПолныеИменаОбрабатываемыхОбъектов Цикл 
		...
		//++Сов 
		Если МетаданныеОбъекта = Метаданные.Справочники.ЗадачиПроектовПрисоединенныеФайлы 
			или МетаданныеОбъекта = Метаданные.Справочники.ИнцидентыПрисоединенныеФайлы 
			или МетаданныеОбъекта = Метаданные.Справочники.ПроектыПрисоединенныеФайлы 
			или МетаданныеОбъекта = Метаданные.Справочники.ПосещенияКонтрагентовПрисоединенныеФайлы Тогда
			Возврат Ложь;	
		КонецЕсли;
		//--Сов
...
Функция СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Очередь, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если МетаданныеОбъекта = Метаданные.Справочники.ЗадачиПроектовПрисоединенныеФайлы 
		или МетаданныеОбъекта = Метаданные.Справочники.ИнцидентыПрисоединенныеФайлы 
		или МетаданныеОбъекта = Метаданные.Справочники.ПроектыПрисоединенныеФайлы 
		или МетаданныеОбъекта = Метаданные.Справочники.ПосещенияКонтрагентовПрисоединенныеФайлы 
		или МетаданныеОбъекта = Метаданные.Справочники.ПосещенияКонтрагентовПрисоединенныеФайлы Тогда
		
		Результат = Новый Структура("ЕстьЗаписиВоВременнойТаблице,ИмяВременнойТаблицы", Ложь, "");
		Результат.ИмяВременнойТаблицы = ДополнительныеПараметры.ИмяВременнойТаблицы;
		Результат.ЕстьЗаписиВоВременнойТаблице = Ложь;
