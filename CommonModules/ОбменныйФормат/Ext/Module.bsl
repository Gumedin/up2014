...
Функция ПолучитьСмету(СтркутураПараметров)
	...
	Запрос.Текст = 
		...
		|	И НЕ БюджетПоМесяцам.ТипСуммы.Порядок = &ТипСуммыПорядок
		|
		|СГРУППИРОВАТЬ ПО
		...
		Запрос.УстановитьПараметр("ТипСуммыПорядок", "1");
...
Функция ПолучитьГрафикОплаты(СтркутураПараметров)
	...
	Запрос.Текст = 
		...
		|	И НЕ Реализация.ВидПлана.Порядок = &ВидПланаПорядок
		|
		|СГРУППИРОВАТЬ ПО
		|	ВЫБОР
		|		КОГДА Реализация.План = 0
		...
		|	Проект";
	 	...
	 	Запрос.УстановитьПараметр("ГодЗадачи", СтркутураПараметров.ГодЗадачи);
		Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДата); 
		Запрос.УстановитьПараметр("ВидПланаПорядок", "0");
		//... добавить условия по вкусу
