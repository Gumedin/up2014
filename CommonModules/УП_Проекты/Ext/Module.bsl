...
Функция РазрешеноСоздаватьДокумент( ДокСсылка, Проект ) Экспорт
	...
	или  ТипЗнч( ДокСсылка ) = Тип("ДокументСсылка.ПланРаботТиражный" ) Тогда
		Если Проект.Статус = Перечисления.СтатусыПроектов.Бланк
		или  Проект.Статус = Перечисления.СтатусыПроектов.Согласование Тогда
			Возврат Истина;
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Проект должен иметь статус " + Перечисления.СтатусыПроектов.Бланк + " или " +
		...
		Возврат (Проект.Статус = Перечисления.СтатусыПроектов.Бланк );
	...
	Тогда
		...
		Иначе
			...
			Сообщение.Текст ="Проект " + Проект  +  Символы.ПС + 
							 "должен иметь статус " + Перечисления.СтатусыПроектов.Бланк + " или " +
					 		  Перечисления.СтатусыПроектов.Согласование;
			Сообщение.Сообщить();
...
Функция СуммаПоСметеЗадачиПроекта( ЗадачаПроекта, СтатьяСметы) Экспорт
	...
	Запрос.УстановитьПараметр("ДоходыФинансовые", СтатьяСметы );
...
&НаСервере
Функция ГрафикОтгрузкиЗадачиДоговорам( ЗадачаПроекта ) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОбеспечениеОплатаОбороты.ЗадачаПроекта КАК ЗадачаПроекта,
		|	ОбеспечениеОплатаОбороты.Месяц КАК ДатаОтгрузки,
		|	СУММА(ОбеспечениеОплатаОбороты.СуммаОбеспеченоОборот) КАК СуммаОтгрузки
		|ИЗ
		|	РегистрНакопления.ОбеспечениеОплата.Обороты КАК ОбеспечениеОплатаОбороты
		|ГДЕ
		|	ОбеспечениеОплатаОбороты.ЗадачаПроекта = &ЗадачаПроекта
