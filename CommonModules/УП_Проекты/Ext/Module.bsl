...
Функция РазрешеноСоздаватьДокумент( ДокСсылка, Проект ) Экспорт
	...
	Если Проект.Статус = Перечисления.СтатусыПроектов.Черновик
		...
		//Согласование изменено на Корректировка по задаче #133086 30.08.2018 ГумединАГ
		или  Проект.Статус = Перечисления.СтатусыПроектов.Прогноз
		или  Проект.Статус = Перечисления.СтатусыПроектов.Корректировка Тогда
			Возврат Истина;
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Проект должен иметь статус " + Перечисления.СтатусыПроектов.Черновик + " или " +
					 			Перечисления.СтатусыПроектов.Корректировка;
			Сообщение.Сообщить();							
...
Функция РазрешеноПроводитьДокумент( ДокСсылка, Проект ) Экспорт
	...
	Если ТипЗнч( ДокСсылка ) = Тип("ДокументСсылка.ПланРабот" ) и 
			Проект.Статус = Перечисления.СтатусыПроектов.ВРаботе Тогда
		Возврат Истина;
	КонецЕсли;
	// 2016 09 19
	// + план работ тиражный
	Если ТипЗнч( ДокСсылка ) = Тип("ДокументСсылка.ПланРаботТиражный" ) и 
			Проект.Статус = Перечисления.СтатусыПроектов.ВРаботе Тогда
		Возврат Истина;
	КонецЕсли;
...
//***************************************************
//Рефакторинг алгоритма определения доступных физ.лиз [#132912]
//&НаСервере
...
&НаСервере
Функция МассивКлиентМенджеров(Год, Подразделение = Неопределено ) Экспорт 
...
//	КонецЕсли;
	...
	Иначе
		...
		Запрос.УстановитьПараметр("ГодЗадачи", Число(Год));

		мФизЛиц = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("МенеджерПроекта");
		Возврат мФизЛиц
...
Функция СуммаПоСметеЗадачиПроекта( ЗадачаПроекта, СтатьяСметы) Экспорт
	Запрос = Новый Запрос;
	//Запрос переписан согласно задаче #133071 от 30.08.2018 ГумединАГ
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(БюджетПоМесяцам.Сумма) КАК СуммаПоСмете,
	...
	//Условие больше не требуется согласно задаче #133071 от 30.08.2018 ГумединАГ
...
Функция ГрафикРеализацииЗадачиДоговорам( ЗадачаПроекта ) Экспорт
	...
	Запрос.Текст = 
		...
		|	Реализация.Договор.Организация КАК Организация
		|ИЗ
		|	РегистрНакопления.Реализация КАК Реализация
		|ГДЕ
		|	Реализация.ЗадачаПроекта = &ЗадачаПроекта
		|	И Реализация.План <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	0,
		|	Реализация.Факт,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыПланаБюджета.Обеспечено),
		|	Реализация.Договор.Организация КАК Организация
		|ИЗ
		|	РегистрНакопления.Реализация КАК Реализация
		|ГДЕ
	...
	тзРеализация.Свернуть("Организация, ДатаРеализации, ВидПлана", "СуммаПлан, СуммаФакт");
