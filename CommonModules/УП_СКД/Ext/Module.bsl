...
Функция СКД_МассивРазрешенныхКлиентМенеджеров(ГодЗадачи) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Проекты.МенеджерПроекта.Ссылка КАК МенеджерПроектаСсылка
		|ИЗ
		|	Справочник.ЗадачиПроектов КАК ЗадачиПроектов
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Проекты КАК Проекты
		|		ПО ЗадачиПроектов.Владелец = Проекты.Ссылка
		|ГДЕ
		|	НЕ Проекты.ПометкаУдаления
		|	И (&ДоступныВсеПроекты
		|			ИЛИ Проекты.Куратор В (&мФизЛицаПользователя)
		|			ИЛИ Проекты.ДиректорПроекта В (&мФизЛицаПользователя)
		|			ИЛИ Проекты.МенеджерПроекта В (&мФизЛицаПользователя))
		|	И ЗадачиПроектов.ГодЗадачи = &ГодЗадачи
		|
		|СГРУППИРОВАТЬ ПО
		|	Проекты.МенеджерПроекта.Ссылка
		|
		|УПОРЯДОЧИТЬ ПО
		|	Проекты.МенеджерПроекта.Наименование";
	
	Запрос.УстановитьПараметр("ГодЗадачи", 	ГодЗадачи);
	Запрос.УстановитьПараметр("ДоступныВсеПроекты", 	СКД_ВсеМенеджеры());
	Запрос.УстановитьПараметр("мФизЛицаПользователя", 	СКД_ДоступныеФЛ());
	
	// 	
	МассивКМ = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("МенеджерПроектаСсылка");
	Возврат МассивКМ;
КонецФункции
...
//////////////////////////////////////////////
// заполняем таблицу значений значениями
// Источник (Текст запроса) 
// тзРезультат (Таблица значение) - Отметка (булево), Значение (Строка)
// Принудительно - заполнение не только пустых списоков
&НаСервере
Процедура СКД_ЗначенияЗапрос(Источник, тзРезультат, Принудительно) Экспорт 
	Если тзРезультат.Количество() = 0 ИЛИ Принудительно Тогда
		тзРезультат.Очистить();
		
		Запрос = Новый Запрос;
		Запрос.Текст = Источник;
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		Пока РезультатЗапроса.Следующий() Цикл
			элемент = тзРезультат.Добавить();
			элемент.Отметка = Истина;
			элемент.Значение = РезультатЗапроса.Ссылка;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
//////////////////////////////////////////////
// заполняем таблицу значений значениями
// Источник (Справочник.<Название>) 
// тзРезультат (Таблица значение) - Отметка (булево), Значение (Строка)
// Принудительно - заполнение не только пустых списоков
&НаСервере
Процедура СКД_ЗначенияСправочника(Источник, тзРезультат, Принудительно, ПустаяСсылка = Неопределено) Экспорт 
	Если тзРезультат.Количество() = 0 ИЛИ Принудительно Тогда
		тзРезультат.Очистить();
...
// Принудительно - заполнение не только пустых списоков
&НаСервере
Процедура СКД_ЗначенияМассив(Источник, тзРезультат, Принудительно) Экспорт 
	Если тзРезультат.Количество() = 0 ИЛИ Принудительно Тогда
		тзРезультат.Очистить();
		...
		Для Каждого строка Из Источник Цикл
			элемент = тзРезультат.Добавить();
			элемент.Отметка = Истина;
			элемент.Значение = строка.Ссылка;
		КонецЦикла;
КонецЕсли;
КонецПроцедуры
