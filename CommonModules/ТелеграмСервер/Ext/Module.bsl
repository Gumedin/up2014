...
Процедура ОбработатьВходящееОбновлениеБота(Обновление, ДанныеБота) Экспорт
	...
	Если ДанныеБота.ОграниченныйДоступ = Истина И ПозволитьДальнейшееОбщение = Ложь Тогда
		...
		// + СИТ 20180126 добавление ID для дальнейшего процесса авторизации
		ТекстСообщенияО = "Доступ к боту ограничен его настройками. Обратитесь к администратору бота (Ваша Заявка: " + ИдентификаторЧата + "), и после получения разрешения заново введите команду /start";		
		Сообщение.Вставить("ТекстСообщения", ТекстСообщенияО);
		// - СИТ
...
// Помещает файл с диска в хранилище присоединенных файлов (ФайловыеФункцииСлужебныйКлиентСервер).
// 
// Параметры:
//  ПолноеИмяФайла                 - Строка - путь к файлу на диске.
//  ВладелецФайла                  - Ссылка на владельца файла. 
//  ПрисоединенныйФайл			   - Возвращаемое значение - заполняется ссылкой
//                                   на добавленный файл.
//
//Процедура ПоместитьВыбранныеФайлыВХранилище(ПолноеИмяФайла,
//                                            ВладелецФайла,
//                                            ПрисоединенныйФайл) Экспорт
											
	//Используется устаревший модуль ФайловыеФункцииСлужебныйКлиентСервер
	//Изменено на РаботаСФайлами											
	Для Каждого ПолноеИмяФайла Из ВыбранныеФайлы Цикл
		ПрисоединенныйФайл = РаботаСФайлами.СоздатьФайлНаОсновеФайлаНаДиске(ВладелецФайла, ПолноеИмяФайла);
	КонецЦикла;
	...
	//Иначе
	//	АдресВременногоХранилищаТекста =
	//		ФайловыеФункцииСлужебныйКлиентСервер.ИзвлечьТекстВоВременноеХранилище(ПолноеИмяФайла);
	//КонецЕсли;
	...
	//ПрисоединенныйФайл = ФайловыеФункцииСлужебныйКлиентСервер.ДобавитьФайл(
	//	ВладелецФайла,
